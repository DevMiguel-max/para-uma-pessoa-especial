...
function goToPage2(delay = 5000){
  if (redirecting) return;
  redirecting = true;
  // fade pra preto no último segundo, depois troca de página
  const fadeMs = reduceMotion ? 0 : 800;
  redirectTimers.push(setTimeout(() => {
    const veil = document.createElement('div');
    veilEl = veil;
    veil.style.cssText = 'position:fixed;inset:0;background:#000;opacity:0;z-index:9999;pointer-events:none;' +
      `transition:opacity ${fadeMs}ms ease;`;
    document.body.appendChild(veil);
    requestAnimationFrame(() => requestAnimationFrame(() => { veil.style.opacity = '1'; }));
  }, Math.max(0, delay - fadeMs)));
  redirectTimers.push(setTimeout(() => { window.location.href = './pagina2/index.html'; }, delay));
}
